...
variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_IMAGE_REGISTRY_CACHE" {
  default = "${VEGITO_DOCKER_TRIXIE_DEBIAN_IMAGE_REGISTRY_CACHE}-trixie-debian-python"
}

variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_IMAGE_LATEST" {
  default = "${VEGITO_DOCKER_PUBLIC_REPOSITORY}/trixie-debian-python:latest"
}

variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_IMAGE_VERSION" {
  default = "${VEGITO_DOCKER_PUBLIC_REPOSITORY}/trixie-debian-python:${VERSION}"
}

variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_IMAGE_DOCKER_BUILDX_LOCAL_CACHE" {
  default = "${VEGITO_DOCKER_TRIXIE_DEBIAN_IMAGE_DOCKER_BUILDX_LOCAL_CACHE}-trixie-debian-python"
}
...
group "vegito-trixie-debian-python-local" {
  targets = [
...
variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_WRITE" {
  default = "type=local,mode=max,dest=${VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE}"
}

variable "VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ" {
  default = "type=local,src=${VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE}"
}
...
target "vegito-trixie-debian-python-dockerd-version-ci" {
  ...
  inherits = ["vegito-trixie-debian-python-base"]
  tags = [
    VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_VERSION,
  ]
  cache-from = concat(
    USE_REGISTRY_CACHE ? [
      "type=registry,ref=${VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_REGISTRY_CACHE}"
    ] : [],
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_READ
    ] : [],
    [
      VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_LATEST
    ]
  )
  cache-to = concat(
    ENABLE_LOCAL_CACHE ? [
      VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_DOCKER_BUILDX_LOCAL_CACHE_WRITE
    ] : [],
  )
  platforms = platforms
}
...
target "vegito-trixie-debian-python-dockerd-latest-ci" {
  ...
  cache-to = [
    USE_REGISTRY_CACHE ? "type=registry,ref=${VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_IMAGE_REGISTRY_CACHE},mode=max" : "",
    "type=inline"
  ]
  platforms = platforms
}
...
target "vegito-trixie-debian-python-dockerd-desktop-x-version-ci" {
  ...
  contexts = {
    debian = "target:vegito-trixie-debian-dockerd-desktop-x-version-ci"
  }
  tags = [
...
target "vegito-trixie-debian-python-dockerd-desktop-x-latest-ci" {
  ...
  contexts = {
    debian = "target:vegito-trixie-debian-dockerd-desktop-x-latest-ci"
  }
  tags = [
...
target "vegito-trixie-debian-python-dockerd-desktop-x" {
  ...
  contexts = {
    debian = "target:vegito-trixie-debian-dockerd-desktop-x"
  }
  tags = [
    VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_DESKTOP_X_IMAGE_LATEST,
    VEGITO_DOCKER_TRIXIE_DEBIAN_PYTHON_DOCKER_DESKTOP_X_IMAGE_VERSION,

...
if [ ${VEGITO_DESKTOP_X_CONTAINER_DISPLAY_START:-"true"} = "true" ]; then
...
if command -v nvidia-smi >/dev/null 2>&1 &&
    ...
    if [ -z "${VEGITO_DESKTOP_X_GPU_MODE:-}" ]; then
        echo "✅ NVIDIA GPU acceleration detected -> using Wayland GPU mode"
        export VEGITO_DESKTOP_X_GPU_MODE="wayland"
    fi
fi

if [ -z "${VEGITO_DESKTOP_X_GPU_MODE:-}" ]; then
    export VEGITO_DESKTOP_X_GPU_MODE="swiftshader_indirect"
    echo "ℹ️ No GPU acceleration detected -> using SwiftShader fallback"
fi

case "${VEGITO_DESKTOP_X_GPU_MODE}" in
    "host")
    ...
    *)
        echo "⚠️ Unknown VEGITO_DESKTOP_X_GPU_MODE='${VEGITO_DESKTOP_X_GPU_MODE}', falling back to SwiftShader"
        display-start.sh &
        bg_pids+=("$!")
        ;;
esac
